...
void readPPMHeader(FILE *fp, int *width, int *height)
{
  ...
  while (ch == '#')
	...
	ch = getc(fp);
      ...
      ch = getc(fp);            /* check first char of next line */
    }
...
Image* imageCopy(Image *original) {
  ...
  Image *newImage = imageCreate(width, height);

  for (int x=0; x<width; x++) {
    for (int y=0; y<height; y++) {

      imageSetPixel(newImage, x, y, 0, imageGetPixel(original, x, y, 0));
      imageSetPixel(newImage, x, y, 1, imageGetPixel(original, x, y, 1));
      imageSetPixel(newImage, x, y, 2, imageGetPixel(original, x, y, 2));
